...
import { createChecklistStore as createChecklist, listObrasStore, type ChecklistItem, type Obra } from "@/lib/store";
import { getSession } from "@/lib/session";
import { exportChecklistToExcel } from "@/lib/excel-export";
import { shareViaWhatsApp } from "@/lib/whatsapp";
import { toast } from "sonner";
import {
  ArrowLeft, FileText, Loader2, Sparkles, Send, Building2, MapPin, Truck,
  User, Hash, Calendar, FileCheck, ChevronDown, ChevronUp, Download,
  MessageCircle, QrCode, ShieldCheck, AlertTriangle, CheckCircle2, XCircle, Package,
} from "lucide-react";
...
function UploadPage() {
  ...
  async function generate() {
    if (!chave && items.length === 0) { toast.error("Analise primeiro o PDF"); return; }
    setSaving(true);
    try {
        ...
        <div className="bg-card rounded-2xl border p-4 space-y-3">
          ...
          {obraAtualNome && (
            <p className="text-xs text-muted-foreground flex items-center gap-1.5">
              ...
              Obra selecionada: <span className="font-semibold text-foreground">{obraAtualNome}</span>
            </p>
          )}
        </div>
          ...
          <div className="space-y-4 animate-fade-in-up">
            ...
            {extracted.chave_at_needs_validation && (
              <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800">Chave AT necessita validação</p>
                  <p className="text-xs text-amber-600">Introduza manualmente a Chave AT do documento</p>
                </div>
              </div>
            )}
            ...
            <div className={`rounded-xl p-3 border flex items-center gap-3 ${
              ...
              <FileText className={`size-5 shrink-0 ${extracted.tipo_guia === "devolucao" ? "text-orange-600" : "text-blue-600"}`} />
              <div>
                <p className={`text-xs font-bold uppercase tracking-wider ${extracted.tipo_guia === "devolucao" ? "text-orange-700" : "text-blue-700"}`}>
                  Documento detetado
                </p>
                <p className="text-sm font-semibold">{extracted.tipo_documento || (extracted.tipo_guia === "devolucao" ? "Nota/Guia de Devolução" : "Guia de Transporte")}</p>
              </div>
            </div>
              ...
              <div className="bg-secondary/10 border border-secondary/30 rounded-xl p-4 space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-xs uppercase tracking-wider text-secondary font-semibold">QR Code Detetado</p>
                    <p className="font-mono text-sm font-bold">Lido com sucesso</p>
                  </div>
                  <span className="text-xs bg-secondary text-secondary-foreground px-2 py-1 rounded-full font-semibold">{extracted.qr_confidence}%</span>
                </div>
                {extracted.qr_at_code && (
                  <p className="text-xs text-muted-foreground">
                    AT Code: <span className="font-mono font-semibold text-foreground">{extracted.qr_at_code}</span>
                  </p>
                )}
                {extracted.atcud && (
                  <p className="text-xs text-muted-foreground">
                    ATCUD: <span className="font-mono font-semibold text-foreground">{extracted.atcud}</span>
                  </p>
                )}
              </div>
            )}
            ...
            {extracted.validations.length > 0 && (
              <div className="bg-card rounded-2xl border overflow-hidden">
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">{extracted.processing_time}ms</span>
                  ...
                  <div className="px-4 pb-4 space-y-1.5 border-t pt-3 max-h-60 overflow-y-auto">
                    {extracted.validations.map((v, i) => (
                      <div key={i} className="flex items-start gap-2 text-sm">
                        {v.status === "ok" && <CheckCircle2 className="size-4 text-green-600 shrink-0 mt-0.5" />}
                        {v.status === "warning" && <AlertTriangle className="size-4 text-amber-500 shrink-0 mt-0.5" />}
                        {v.status === "error" && <XCircle className="size-4 text-red-500 shrink-0 mt-0.5" />}
            ...
            <div className="bg-card rounded-2xl border p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs uppercase tracking-wider text-muted-foreground">N. Guia</label>
                  <input
                    value={numeroGuia}
                ...
                <div>
                  <label className="text-xs uppercase tracking-wider text-muted-foreground">V/N Contrib.</label>
                  <input
            ...
            {/* Emissor */}
            <Collapsible icon={<Building2 className="size-5 text-secondary" />} title="Dados do Emissor" open={showEmissor} toggle={() => setShowEmissor(!showEmissor)}>
              ...
              {extracted.emissor.capital_social && <InfoRow icon={<Hash className="size-4" />} label="Capital Social" value={extracted.emissor.capital_social} />}
            </Collapsible>

            {/* Destinatário */}
            <Collapsible icon={<User className="size-5 text-secondary" />} title="Dados do Destinatário" open={showDestinatario} toggle={() => setShowDestinatario(!showDestinatario)}>
            ...
            {/* Transporte */}
            <Collapsible icon={<Truck className="size-5 text-secondary" />} title="Dados de Transporte" open={showTransporte} toggle={() => setShowTransporte(!showTransporte)}>
              ...
              {extracted.transporte.disponibilizacao && <InfoRow icon={<Calendar className="size-4" />} label="Disponibilização" value={extracted.transporte.disponibilizacao} />}
              {extracted.transporte.certificacao && <InfoRow icon={<FileCheck className="size-4" />} label="Certificação" value={extracted.transporte.certificacao} />}
            </Collapsible>
            ...
            <div className="bg-card rounded-2xl border p-4 space-y-3">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                  <span className="size-7 bg-primary text-primary-foreground rounded-lg flex items-center justify-center text-xs font-bold">{items.length}</span>
                  Artigos detetados
                </p>
              ...
              <ul className="divide-y rounded-xl border overflow-hidden">
                ...
                {items.length === 0 && <li className="p-4 text-sm text-muted-foreground text-center">Sem artigos detetados</li>}
              </ul>
            </div>
      ...
      {(chave || items.length > 0) && extracted && (
        <div className="fixed bottom-0 inset-x-0 bg-background/95 backdrop-blur-lg border-t shadow-2xl p-4 space-y-3 z-50">
          <div className="grid grid-cols-3 gap-3">
            <button
              onClick={() => { exportChecklistToExcel(buildChecklist()); toast.success("Excel exportado"); }}
            ...
            >
              {saving ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />} Guardar
            </button>
          </div>
        </div>
      )}
...
function Collapsible({
  icon, title, open, toggle, children,
}: {
  icon: React.ReactNode; title: string; open: boolean; toggle: () => void; children: React.ReactNode;
}) {
  return (
    <div className="bg-card rounded-2xl border overflow-hidden">
      <button type="button" onClick={toggle} className="w-full flex items-center gap-2 p-4 text-left hover:bg-muted/30 transition">
        {icon}
        <span className="flex-1 font-bold text-sm uppercase tracking-wider">{title}</span>
        {open ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
      </button>
      {open && <div className="px-4 pb-4 space-y-2 text-sm border-t pt-3 animate-fade-in-up">{children}</div>}
    </div>
  );
}
